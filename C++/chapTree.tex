...
public:
    ...
    bool isSymmetric(TreeNode *left, TreeNode *right) {
        if (!left and !right) return true;  // 终止条件
        if (!left or !right) return false;  // 剪枝
        if (left->val != right->val) return false;  // 剪枝
...
public:
    ...
    int balancedHeight (TreeNode* root) {
        ...
        if (lhs < 0 or rhs < 0 or std::abs(lhs - rhs) > 1) return -1;  // 剪枝
