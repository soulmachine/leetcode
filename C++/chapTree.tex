    ...
    vector<int> inorderTraversal(TreeNode *root) {
        ...
        while (cur != NULL) {
            if (cur->left == NULL) {
                ...
                while (node->right != NULL and node->right != cur)
                    node = node->right;
...
// 分治法，类似于 Convert Sorted Array to Binary Search Tree，自顶向下，复杂度O(nlogn)
class Solution {
public:
...
public:
    ...
    TreeNode* buildTree(BidiIt in_first, BidiIt in_last,
        ...
        if (std::distance(in_first, in_last) == 0) return NULL;
        if (std::distance(post_first, post_last) == 0) return NULL;
